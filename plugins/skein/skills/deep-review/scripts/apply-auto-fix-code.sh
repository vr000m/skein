...
# af_same_import_symbol_set so the auditor (audit-auto-fix-eligibility.sh)
# uses the exact same normalisation. The import_sort kind gate calls the
# `af_` helpers directly.
...
while IFS= read -r finding; do
	...
	if [[ "$kind" == "docstring_typo" ]]; then
		...
		if ! af_same_import_symbol_set "$stripped_before" "$stripped_after"; then
