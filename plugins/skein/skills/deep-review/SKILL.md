...
REPO_ROOT="$(git rev-parse --show-toplevel)"
RUN_ID="<run_id>"   # the run id resolved before dispatch; a --continue reuses the prior one
# BASE_COMMIT, HEAD_COMMIT, DIFF_HASH and REVIEW_FOCUS_HASH must be bound in this
# same call too: the commits and the diff hash come from Step 0 (Resolve Identity
# and Check Scope), the review-focus hash from Step 1a (Pre-dispatch Banner).
"${CLAUDE_PLUGIN_ROOT}"/skills/deep-review/scripts/collect-lens-results.sh --root "$REPO_ROOT" --skill deep-review --run-id "$RUN_ID" --expected-file "$REPO_ROOT/.deep-review/lenses/$RUN_ID/expected.json" [--attempts "<lens>:<n>" ...] [--running "<lens>:<n>" ...] \
