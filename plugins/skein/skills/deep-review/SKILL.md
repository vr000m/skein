...
```
"${CLAUDE_PLUGIN_ROOT}"/skills/deep-review/scripts/collect-lens-results.sh --root "<repo-root>" --skill deep-review --run-id "<run_id>" --expected-file "<repo-root>/.skein/lens-runs/<run_id>/expected.json" [--attempts "<lens>:<n>" ...] [--running "<lens>:<n>" ...]
```
...
- **`partial` or `missing`** — respawn that lens **once**: same prompt template, `{{UNITS}}` narrowed to the collector's `unreviewed` list (still a JSON array), `--attempt 2`. Rewrite the units file with the narrowed list before re-collecting — it is the `--expected-file` the next collect reads, so a stale file would re-report the units the respawn is no longer covering. If you write the attempt-2 `start` record on the lens's behalf, use `persist-lens-result.sh --json-file <path>`. Re-run `collect-lens-results.sh` after the respawn to fold in the attempt-2 results, then re-persist.
