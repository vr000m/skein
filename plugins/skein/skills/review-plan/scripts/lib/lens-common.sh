#!/usr/bin/env bash
# lens-common.sh — Phase 2 lens-persistence helpers: the path, id/unit and
# append primitives the two LENS scripts share.
#
# TWO callers. Keep this list exact: it is the only place that records which
# helpers exist for whom, and `grep -rn 'lens-common.sh' scripts/ plugins/`
# must return exactly these sourcing sites (plus the bundled copies of the
# same two scripts).
#   scripts/persist-lens-result.sh      (writer)
#   scripts/collect-lens-results.sh     (reader/merge)
#
# WHY THIS IS A SEPARATE FILE (R11/F3). These helpers were the back half of
# scripts/lib/persist-common.sh, which is sourced by FOUR scripts — the two
# above plus the two STATE-FILE scripts (persist-review-state.sh,
# persist-deep-review-state.sh). The state scripts call NONE of them: they
# carried ~470 lines of lens path/charset logic they never execute, and the
# combined file's "which helpers belong to whom" header was the only thing
# keeping the two halves apart. Splitting makes the boundary structural
# instead of documentary.
#
# The split is a MOVE, not a rewrite: every function below is byte-identical
# to its pre-split form, prefix included. `persist_` is deliberately kept —
# renaming would touch every call site to say nothing new.
#
# This file sources scripts/lib/persist-common.sh itself, for the three
# validators genuinely shared with the state scripts (persist_require_value,
# persist_validate_json_shape, persist_assert_no_duplicate_keys) plus
# persist_root_dir, which collect-lens-results.sh uses for its --root
# fallback. A caller therefore sources THIS file only, never both.
#
# Like persist-common.sh, this file assumes the caller has already sourced
# scripts/lib/auto-fix-common.sh (af_assert_no_symlink).
#
# Helpers provided:
...
# Pull in the genuinely-shared half. Resolved from THIS file's own directory
# (the gate-bounded.sh:190 idiom) rather than from $PWD or the caller's
# location, so a bundled copy under plugins/*/skills/*/scripts/lib/ finds its
# sibling and not the canonical one.
_LENS_COMMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=scripts/lib/persist-common.sh
. "$_LENS_COMMON_DIR/persist-common.sh"
unset _LENS_COMMON_DIR
...
#
# "Deliberately not consolidated" is not the same as "unguarded" (R11/F13).
# Nothing used to fail when a new skill was added to three of the four, so
# the decision above was paid for with a silent partial-registration risk.
# tests/parity/test-state-dir-registration.sh now asserts the four sites list
# an IDENTICAL skill set, which closes that gap without unwinding the
# decision. Adding a skill means editing all four sites AND watching that
# test go green.
