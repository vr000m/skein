...
3. Enumerate the tasks with `fan-out.sh tasks`, and pin the plan revision. Do NOT
   ...
   "${CLAUDE_PLUGIN_ROOT}/skills/fan-out/fan-out.sh" tasks --plan "$PLAN" --plan-sha256 "$PLAN_SHA256" "$REPO_ROOT"
   ```
...
Phase 1 already checked that `${CLAUDE_PLUGIN_ROOT}/skills/fan-out/fan-out.sh` resolves
and printed the plan's task records and `PLAN_SHA256`. That check binds nothing for the
steps below: shell variables do not survive from one Bash tool call to the next, so every
`fan-out.sh` call spells the full `${CLAUDE_PLUGIN_ROOT}` path and re-binds what it needs
itself.
...
1. **Create worktree** (`N` is the ordinal, `<plan-sha256>` the digest from Phase 1, both
   ...
   WORKTREE=$("${CLAUDE_PLUGIN_ROOT}/skills/fan-out/fan-out.sh" setup "$BASE_BRANCH" --plan "$REPO_ROOT/<plan-path-relative-to-the-repo-root>" --task-id N --plan-sha256 <plan-sha256> "$REPO_ROOT")
   ```
