...
1. **Header** - Status, **Component**, assignee, priority, branch, dates, objective. For `**Component**`: name the subsystem or skill area this plan touches (it groups the plan in `/plan-view` and the `dev_plans/README.md` index). Reuse an existing component value when one fits — scan sibling plans' `**Component**` fields (`rg '^\*\*Component\*\*' docs/dev_plans/`) and prefer an existing label over inventing a near-duplicate. Infer a sensible value from the objective/slug and confirm with the user. Omit only if genuinely cross-cutting; plan-view will group it as `(uncategorized)`.
   - `**Review Gates:**` *(optional)* — `none | full`, default `none`. An above-marker header contract field (plans carry no YAML frontmatter — this is an inline markdown header line, written the same way as `**Status**`/`**Component**`/`**Branch**`). `/conduct` and `/fan-out` read this field at their respective terminal seams to decide whether to auto-chain `review-gauntlet`: `full` runs all logical gate slots (Codex reports unsupported slots as gated/deferred). Absent or `none` is a strict no-op. There is no `quick` value on the Claude side — `review-gauntlet` has no gate `/code-review` can run through; run `/code-review xhigh --fix` yourself for a fast pass. Any other value (including the retired `quick`, or a typo) is treated as `none` by both Claude-side readers (`conduct` and `fan-out`), but with a warning naming the unrecognized value — it never silently claims to have run. This is Claude-only: the Codex mirror still supports `quick` and is unaffected, so a plan authored with `quick` runs a gate on Codex and no-ops (with a warning) on Claude. Because the field sits above the review marker, changing it after `/review-plan` invalidates the marker hash and forces re-review — this is intended: opting a plan into automated review gates is a contract decision, not a runtime tweak.
2. **Context** - Background, why this work is needed
3. **Requirements** - Specific requirements and constraints
4. **Implementation Checklist** - Phased contract section. Each phase should include a contract block directly under the heading with `**Impl files:**`, `**Test files:**`, `**Test command:**` (in backticks), and optionally `**Validation cmd:**` (in backticks, runs after tests pass, failure hands back to user) and `**Goal:**` (optional, 1-2 lines, plain prose — not backticked). `/conduct` reads these to decide spawn strategy and run tests/validation. Keep phase tasks as plain bullets; per-phase completion and durable findings live below the review marker in `## Progress` and `## Findings`, so runtime edits do not invalidate the marker. Omit all slots only when the phase is not `/conduct`-driven; if every phase omits them, `/conduct` emits a degraded-mode warning.
   - `**Goal:**` *(optional)* — a 1-2 line statement of the phase's design intent, invariant, or quality bar (e.g. "Marker hash must cover contract bytes only; workspace edits never invalidate it"). It sits in the phase contract block alongside the other slots, above the review marker — it is part of the immutable contract, so adding or editing it on an already-reviewed plan invalidates the marker hash and forces re-review, the same as any other contract-slot edit. Absent `**Goal:**` is a no-op: the phase runs with no design-intent directive injected. This is the single design-intent source read by both `/conduct`'s implementer/test-writer injection (`{{PHASE_GOAL}}`) and `review-gauntlet`'s fixer (Guardrail 1, quarantine-vs-fix decision) — one field, two consumers, no divergent intent channel.
...
- **Model**: `sonnet`, **Effort**: `medium` (balanced/low-cost planner tier — light pattern reasoning over fact-gathering; mechanical work per the two-tier policy, `AGENTS.md` Model/Effort Policy) <!-- sonnet/medium: mechanical fact-gathering, not judgment work -->
