...
1. **Header** - Status, **Component**, assignee, priority, branch, dates, objective. For `**Component**`: name the subsystem or skill area this plan touches (it groups the plan in `/plan-view` and the `dev_plans/README.md` index). Reuse an existing component value when one fits — scan sibling plans' `**Component**` fields (`rg '^\*\*Component\*\*' docs/dev_plans/`) and prefer an existing label over inventing a near-duplicate. Infer a sensible value from the objective/slug and confirm with the user. Omit only if genuinely cross-cutting; plan-view will group it as `(uncategorized)`.
   - `**Review Gates:**` *(optional)* — `none | quick | full`, default `none`. An above-marker header contract field (plans carry no YAML frontmatter — this is an inline markdown header line, written the same way as `**Status**`/`**Component**`/`**Branch**`). `/conduct` and `/fan-out` read this field at their respective terminal seams to decide whether to auto-chain `review-gauntlet`: `quick` runs the code-review gate only; `full` runs all logical gate slots (Codex reports unsupported slots as gated/deferred). Absent or `none` is a strict no-op — current behavior unchanged. Because the field sits above the review marker, changing it after `/review-plan` invalidates the marker hash and forces re-review — this is intended: opting a plan into automated review gates is a contract decision, not a runtime tweak.
