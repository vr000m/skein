...
- Check `git rev-parse --verify --quiet "refs/tags/vX.Y.Z"` (local) and `git ls-remote --exit-code --tags origin "refs/tags/vX.Y.Z"` (origin, after `git fetch --tags origin`). Record which case applies — Step 5 reuses this determination rather than re-checking.
