...
Resolve the exact commit this run will tag before asking for confirmation: run `git status --porcelain` and stop if the worktree is dirty — uncommitted changes make it ambiguous whether `HEAD` is actually the commit the user intends to release, and a tag/release is hard to undo once pushed. Then run `git rev-parse --abbrev-ref HEAD` and `git rev-parse HEAD` to record the current branch and commit SHA that Step 5 will tag (or, in the re-sync case, verify the existing tag against).

Per this repo's own `CLAUDE.md` (Executing Actions With Care), tag pushes and release publishes are external, hard-to-reverse actions — show the user the computed title and full body, the branch and commit SHA that will be tagged, and state whether this is a **new tag+release** or a **re-sync of an existing release**, and get explicit confirmation before running any `git tag`/`git push`/`gh release` command. Do not proceed silently.

### Step 5: Create or Re-Sync the Tag

Check both local and remote existence — the tag must exist on **origin**, not just locally, before Step 6 (`gh release create --verify-tag` requires it):

- If `git rev-parse --verify --quiet "refs/tags/vX.Y.Z"` finds nothing locally, create the tag and push it: `git tag -a vX.Y.Z -m "<repo> vX.Y.Z"` then `git push origin vX.Y.Z`.
- If the tag exists locally but `git ls-remote --exit-code --tags origin "refs/tags/vX.Y.Z"` finds nothing on origin (a local-only tag — e.g. created outside this skill, or by a prior interrupted run), push it now: `git push origin vX.Y.Z`. Do not silently proceed with an unpushed tag.
- If the tag already exists on origin, **first verify it points at the commit confirmed in Step 4** — run `git rev-parse "vX.Y.Z^{commit}"` (after `git fetch --tags origin`) and compare it to Step 4's recorded `HEAD` SHA. If they match, reuse the tag as-is — do not re-create or force-move it. If they differ, stop and tell the user the existing tag points at a different commit than the one they just confirmed (see Edge Cases: "target version's tag exists but points at an unexpected commit") — do not silently reuse or force-move it.

### Step 6: Create or Edit the Release

Reuse the existence outcome Step 3.1 already recorded — do not re-run `gh release view`. Nothing state-changing happens between Step 3.1 and here (Step 4 is a confirmation prompt, Step 5 only creates/pushes/verifies the tag, neither touches release existence):

- If Step 3.1 recorded no release exists (`release not found`), run `gh release create vX.Y.Z --verify-tag --title "<title>" --notes "<body>"`. `--verify-tag` aborts the command instead of creating a release against a tag absent from origin — a defensive backstop on top of Step 5's own push, in case the tag disappeared or was never actually pushed.
- If Step 3.1 recorded that a release already exists (the retrofit/re-sync case), run `gh release edit vX.Y.Z --title "<title>" --notes "<body>"` instead. This makes the skill idempotent on body content and safe to re-run against an already-published release to bring it into the canonical shape.
...
1. **Tags**: `git fetch --tags origin` first (Step 2's own freshness note applies here too — the inventory must reflect origin, not a possibly-stale local clone), then `git tag --list --sort=v:refname`.
