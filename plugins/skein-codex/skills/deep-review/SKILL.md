...
1. **Collect lens output as JSON-Lines.** Produce the stream from disk:
   ...
   ```
   "$SKILL_DIR"/scripts/collect-lens-results.sh --root "$REPO_ROOT" --skill deep-review --run-id "$RUN_ID" --expected-file "$REPO_ROOT/.deep-review/lenses/$RUN_ID/expected.json" [--attempts "<lens>:<n>" ...] --findings-jsonl > findings.jsonl
   ```
...
```
"$SKILL_DIR"/scripts/collect-lens-results.sh --root "$REPO_ROOT" --skill deep-review --run-id "$RUN_ID" --expected-file "$REPO_ROOT/.deep-review/lenses/$RUN_ID/expected.json" [--attempts "<lens>:<n>" ...] [--running "<lens>:<n>" ...] | "$SKILL_DIR"/scripts/persist-deep-review-state.sh --harness codex --run-id "$RUN_ID" --base-commit "$BASE_COMMIT" --head-commit "$HEAD_COMMIT" --diff-hash "$DIFF_HASH" --review-focus-hash "$REVIEW_FOCUS_HASH" --from-collector
```
