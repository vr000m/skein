...
# repo. Outside a worktree only the ledger path and its immediate parent are
# checked — the two positions the threat actually occupies — for the same
# reason.
#
# ANCESTORS THAT DO NOT EXIST YET ARE STILL WALKED. `write_fresh_ledger` calls
# this guard and then `mkdir -p`, so the components `mkdir -p` is about to
# create are exactly the ones an attacker wants a symlink under. An ancestor
# that cannot be canonicalised is NOT evidence of safety: the walk continues
# upward with containment undecided until it reaches one that exists.
#
...
ledger_assert_no_symlink() {
	...
	#
	# There is no legitimate caller that needs it -- `--ledger` is composed
	# from a repo root and a fixed subpath -- so the fail-closed answer is to
	# reject the SHAPE. The `/$path/` wrapping makes the slash-delimited
	# match cover the first and last components too, so an ordinary filename
	# that merely contains dots (`a..b.json`) is untouched.
	case "/$path/" in
	*/../*)
		echo "convergence-ledger: refusing a ledger path containing '..': $path" >&2
		return 1
		;;
	esac
	...
	#   perfectly legitimate `--init` was refused.
	#
	# Codex addendum (:508) — the containment probe treated an ancestor it
	#   could not `cd` into as PROOF OF SAFETY (`|| return 0`). For
	#   `--ledger "$repo/link/sub/new/ledger.json"` with `link` a symlink out
	#   of the repo, `$repo/link/sub/new` does not exist, the probe bailed out
	#   with success, and `mkdir -p` then followed `link` and wrote the ledger
	#   outside the worktree. A not-yet-existing ancestor is exactly what
	#   `mkdir -p` is about to create: it must be WALKED, never trusted.
	#
	...
	#
	#   Pass 1 climbs from the ledger's parent looking for an ancestor whose
	...
	#
	#   Pass 2 -L-tests every component from the ledger's parent up to, but
