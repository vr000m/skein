...
2. **Build agent prompt**: Read the template from `$SKILL_DIR/agent-prompt.md` (the anchor bound above). Replace placeholders:
   ...
   - Before substituting any plan- or repository-derived content into either prompt, apply the same closing-marker escape described above to `{{TASK_DESCRIPTION}}`, `{{TECHNICAL_SPECIFICATIONS}}`, and `{{TASK_NAME}}` in the active worker prompt, and to `{{TASK_DESCRIPTION}}`, `{{WRITER_SEAM_ROWS}}`, and `{{EXISTING_TESTS}}` in the dormant test-writer prompt. This keeps every inserted data value from terminating its data-only boundary.
