...
```
"$SKILL_DIR"/scripts/collect-lens-results.sh --root "<repo-root>" --skill review-plan --run-id "<run_id>" --expected-file "<repo-root>/.skein/lens-runs/<run_id>/expected.json" --attempts "<lens>:<n>" ... [--running "<lens>:<n>" ...]
```
...
1. **Collect lens output as JSON-Lines.** Produce the stream from disk:
   ...
   ```
   "$SKILL_DIR"/scripts/collect-lens-results.sh --root "<repo-root>" --skill review-plan --run-id "<run_id>" --expected-file "<repo-root>/.skein/lens-runs/<run_id>/expected.json" [--attempts "<lens>:<n>" ...] --findings-jsonl > findings-lenses.jsonl
   ```
