...
2. Present a plain-text prompt with exactly three choices: **accept**, **propose an alternative**, or **waive**. Ask the user to reply with one of those choices in text.
3. If the user picks **propose an alternative**, follow up with a free-text prompt to capture the override — mirroring `/review-plan` Clarify's existing free-text path for findings with no clear fixed options.
...
- **Target is an existing file that is not a `docs/dev_plans/*.md` plan** (for example `README.md`, `AGENTS.md`, or any other project doc): treat it strictly as a read-only interview target. Do not call `/dev-plan update` and do not edit the file directly for any outcome — there is no plan-update route or `### Review Waivers` section for a non-plan file to write into. Summarize every resolved decision (`accept`/`override`/`waive`) back in the conversation, and offer to run `/dev-plan create` if the user wants the resolved design persisted as a new plan.
