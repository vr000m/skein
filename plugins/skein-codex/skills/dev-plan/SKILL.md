...
1. **Header** - Status, **Component**, assignee, priority, branch, dates, objective. For `**Component**`: name the subsystem or skill area this plan touches (it groups the plan in `/plan-view` and the `dev_plans/README.md` index). Reuse an existing component value when one fits — scan sibling plans' `**Component**` fields (`rg '^\*\*Component\*\*' docs/dev_plans/`) and prefer an existing label over inventing a near-duplicate. Infer a sensible value from the objective/slug and confirm with the user. Omit only if genuinely cross-cutting; plan-view will group it as `(uncategorized)`.
   - `**Review Gates:**` *(optional)* - `none | quick | full`, default `none`. An above-marker header contract field (plans carry no YAML frontmatter - this is an inline markdown header line, written the same way as `**Status**`/`**Component**`/`**Branch**`). Codex `/conduct` and `/fan-out` read this field at their respective terminal seams to decide whether to auto-chain `review-gauntlet`: `quick` runs the native Codex code-review gate only; `full` runs the Codex gate matrix, including explicit gated/deferred entries for unsupported slots. The `quick` value is Codex-only; Claude-side `conduct`/`fan-out` readers treat it as `none` with a warning. Absent or `none` is a strict no-op. Because the field sits above the review marker, changing it after `/review-plan` invalidates the marker hash and forces re-review - this is intended: opting a plan into automated review gates is a contract decision, not a runtime tweak.
2. **Context** - Background, why this work is needed
3. **Requirements** - Specific requirements and constraints
4. **Implementation Checklist** - Phased contract section. Each phase should include a contract block directly under the heading with `**Impl files:**`, `**Test files:**`, `**Test command:**` (in backticks), and optionally `**Validation cmd:**` (in backticks, runs after tests pass or after a no-test phase, failure hands back to user) and `**Goal:**` (optional, 1-2 lines, plain prose - not backticked). `/conduct` reads these to decide how to spawn subagents and run tests/validation. Keep phase tasks as plain bullets; per-phase completion and durable findings live below the review marker in `## Progress` and `## Findings`, so runtime edits do not invalidate the marker. Omit all slots only when the phase is not `/conduct`-driven; if every phase omits them, `/conduct` emits a degraded-mode warning.
   - `**Goal:**` *(optional)* - a 1-2 line statement of the phase's design intent, invariant, or quality bar (e.g. "Marker hash must cover contract bytes only; workspace edits never invalidate it"). It sits in the phase contract block alongside the other slots, above the review marker - it is part of the immutable contract, so adding or editing it on an already-reviewed plan invalidates the marker hash and forces re-review, the same as any other contract-slot edit. Absent `**Goal:**` is a no-op: the phase runs with no design-intent directive injected. This is the single design-intent source read by both Codex `/conduct`'s implementer/test-writer injection (`{{PHASE_GOAL}}`) and `review-gauntlet`'s fixer (Guardrail 1, quarantine-vs-fix decision) - one field, two consumers, no divergent intent channel.
...
6. Weave Explore facts (verified paths, observed patterns, dependency versions, verified git refs) into Technical Specifications / Files-to-Modify. Identify files to modify, potential risks, and any Review Focus items that should be written into the plan.
...
On `skein:dev-plan create`, after Explore returns and **before phases are written**, decide whether the plan needs an `## Architecture & Call Flow` section. If it does, draft the section and stop at a user confirmation gate before continuing into phase definition.

**When to include it.** Include the section only when the system has **2 or more independently-executing components** — for example, an LLM SDK/API call, an LLM router, an MCP server, a `spawn_agent` subagent, a CLI front-end, or a storage layer. Count components that run in their own execution context and hand control and context to one another. **Omit the section entirely** for single-component changes such as a typo fix, isolated script tweak, or one-file refactor. When in doubt, count distinct call-graph nodes; 2+ means include.

**The gate.** The section encodes cross-component assumptions, so draft it **before** writing phases and present it to the user for confirmation: show the draft section and ask "does this look correct?". Revise based on the answer, then continue to phase definition. This gate applies to `create` only.
...
- A node label that starts with `/` or `\` right after the opening bracket (e.g. `DPU[/dev-plan update]` for a node naming the `/dev-plan update` command) is read as the start of a parallelogram/trapezoid shape and fails when there is no matching closing `/]`.
...
These diagrams frequently label nodes and edges with commands and `{{PLACEHOLDER}}` tokens, so wrap any label containing `/`, `\`, or `{{...}}` in quotes: `DPU["/dev-plan update"]`, `-->|"{{PHASE_GOAL}}"|`. Verify by checking the rendered diagram in `/plan-view` output, not just by eye.
...
When `spawn_agent` is unavailable, the inline fallback does not add a separate worker call, but it still follows the same prompt, structured-fact contract, and rubric self-check. `/dev-plan update` and `/dev-plan complete` do not re-run Explore, so they cost nothing beyond the main agent's own turn.
