...
- **`skein:review-gauntlet` (Claude side, Phases 1-6).** A new conductor skill that chains the review gates (`/code-review`, adversarial Codex review, `skein:deep-review`, `/security-review`) into one convergence loop: the multi-spawn gates run at the conductor's own top level (they already fan out their own verifier/lens subagents and forbid nested spawning), while only the fixer batch runs as an isolated clean-context subagent — the dominant repeated context consumer across a 6-12-loop run. Convergence is a pure function in the new bundled `convergence-ledger.sh` (any structural fix restarts from gate 1; all-local fixes take one confirming pass; stop conditions are a clean full pass, a 10-loop cap, or K=2-round non-convergence bail). Two guardrails: design-conflict findings are never auto-fixed (quarantine on `local` blast radius, halt on `structural`), and every non-conflict finding is fixed regardless of confidence score. Reuses the existing bundled auto-fix pipeline rather than forking it — `review-gauntlet` joins `BUNDLE_SKILLS` in `scripts/lib/bundle-map.sh` and gets its own bundled copy of `apply-auto-fix-code.sh` + `reconcile-findings.sh` (called without `--skill`). Opt-in via a new dev-plan header field, `**Review Gates:** none | quick | full` (default `none`); `/conduct`'s terminal step and `/fan-out`'s Phase 6 auto-chain into it only when a plan opts in. New `tests/gauntlet/*` shape/unit/hook tests, wired into `just gauntlet-tests`. Codex mirror lands in Phases C1-C3 (tracked separately).
