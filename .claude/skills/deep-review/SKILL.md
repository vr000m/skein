...
All operative invocations below use `"$SKILL_DIR"/scripts/…`. If `"$SKILL_DIR"/scripts/` is absent, **abort with a clear error** — never fall back to applying fixes by hand or running an unbundled script. The gated applier's safety contract (mandatory `--test-cmd`, one test run per fix, blob restore on failure without touching `HEAD`) holds only when the bundled applier runs.
