...
| release | Yes (user-invoked only) | Yes | Cut or re-sync a GitHub release from a `CHANGELOG.md` section: title `<repo> vX.Y.Z — <highlight>`, body = the section content with its header stripped plus a `**Full diff:**` compare link; idempotent on re-run against an already-published release |
