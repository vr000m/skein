...
2. `skein:review-plan` — Audit the plan by dispatching five parallel fresh-context lens agents (`architecture`, `sequencing`, `spec-and-testing`, `assumptions`, `codebase-claims`); reconciles findings by structural `(file, line, category)` signature and surfaces same-location-different-category findings as cross-references; blocks until complete, and on acceptance writes a review marker footer consumed by `skein:conduct`. Cost: four high-reasoning lenses plus one cheap factual lens per run.
