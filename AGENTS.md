...
just parity-tests                # Bundle + allowlist + orchestration-contract + no-fallback + marker + managed-skill/cleanup-boundary regression coverage. Also runs the release skill's mirror-parity/coverage suites: tests/parity/test-release-lagging-mirror.sh (RELEASE_LAGGING_MIRROR_OK self-tests, below), tests/release/test-lib.sh (fixture git-state builder determinism), tests/release/test-golden-schema.sh (golden key-set/shape lint against tests/release/golden/schema.json), the full release-script-tests group (below), and the pytest release-skill-contract/skill-md-presence/delete-skills suites
...
just release-script-tests         # release skill lib/ scripts (tests/release/test-scripts.sh: goldens, negative cases, exit-code contract, golden tamper self-test; tests/release/test-a2-a25-contract.sh: A2 -> A2.5 stdout field contract against audit-inference.md, with a mutation self-test on both sides; tests/release/test-presence-differential.sh: read-release-template.sh's and resolve-template-marker.sh's independently-implemented Step 1b presence oracles agree on every tests/release/fixtures/presence/state-*.json fixture). Also runs as part of `just parity-tests` (and therefore `just ci`) so local `parity-tests` iteration surfaces regressions here without a full `just ci` run; kept as its own recipe for running just this group in isolation.
...
**Versioning and releases:** skein follows SemVer; changes are recorded in [`CHANGELOG.md`](CHANGELOG.md) (Keep a Changelog format). Tags start at v0.1.0; check the [GitHub releases page](https://github.com/vr000m/skein/releases) or `git tag` for the current list rather than trusting a hardcoded upper bound here. Pin an install to a specific tag with `--ref vX.Y.Z` instead of `--ref main` if you want a stable version rather than tracking `main`.

**Repo-declared release-notes shape:** a target repository may commit `.release-template.json` at its root to override the release skill's default canonical shape (title format, compare-line label, excluded CHANGELOG subsections, `## What's New` presence) — see the release skill's "Repo Template Override" section for the schema. Absence of the file is a byte-for-byte no-op; skein's own repo carries no template and always uses canonical shape.
