...
2. Resolve and validate the host-qualified `ORIGIN_REPO` from agreeing fetch/push URLs, pin every `gh` call with `--repo ORIGIN_REPO`, and derive PREV from a non-mutating, origin-authoritative `git ls-remote --tags origin` snapshot taken before this run can push a tag. Filter to strict `vX.Y.Z`, exclude the target, and select the highest lower version; never use the local tag list as PREV authority.
...
5. Show the exact version, origin-locked repository, target commit/tag state, computed title, and full body at the explicit confirmation gate before any `git tag`, `git push`, or `gh release` mutation. Re-verify the payload, tag/PREV SHAs, release object, and push destination at the mutation-adjacent seams documented by the shipped skill.
...
8. User-invoked only: `disable-model-invocation: true` on the Claude mirror (Codex gets the documented-divergence comment, per Requirement above) — this is an externally-visible, hard-to-reverse action and should never fire autonomously off conversational context.
9. Dual-mirrored under `plugins/skein/skills/release/` and `plugins/skein-codex/skills/release/`, registered in both `MANAGED_SKILLS` lists and both catalogue docs (`README.md`, skills-architecture doc).
...
- `python3 -m pytest tests/parity/test_release_skill_contract.py -q` — locks the mirrored security and behavior contracts, including CHANGELOG's data-only boundary, preserve-absence summary recovery, bounded/fail-closed audit inventory, origin scoping, and file-safe title transport.
...
- `just check-prompt-parity`, `tests/parity/test_skill_md_presence.py`, and `tests/parity/test_release_skill_contract.py` pass.
...
The Requirements, Testing Notes, and Acceptance Criteria above are the current shipped contract. Historical review entries below remain as chronology and may quote superseded intermediate behavior; they are not instructions to resurrect it. The final contract is: CHANGELOG text is untrusted data-only input; repository/release calls are pinned to the validated origin; PREV and remote tag existence come from `git ls-remote --tags origin`; release title is the `name` JSON field; local-only and bare-legacy tags have explicit non-silent handling; title/body mutation transport is file-safe; the audit release inventory is one bounded `--limit 1000` call that fails closed at saturation; and ordinary re-sync preserves an absent What's New block unless the user explicitly chooses to add one. The explicit pre-mutation confirmation gate remains unchanged.
