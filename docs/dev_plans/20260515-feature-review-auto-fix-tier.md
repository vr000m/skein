...
**Known pre-existing renderer bug surfaced:** the awk renderer's TSV decode collapses adjacent empty fields when `IFS=$'\t'` is whitespace-separated, causing the next non-empty value to bleed into an empty slot. Surfaced when an `auto_fix_status` field was preceded by empty `evidence`/`suggestion`. Worked around in the negative-rendering fixture by using non-empty evidence/suggestion strings. The bug is not Phase-5-introduced and should be fixed separately by switching the renderer's intermediate TSV to a non-whitespace field separator (e.g. ASCII `\x1f`).
