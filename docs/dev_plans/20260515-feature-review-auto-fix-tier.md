...
4. **Audit-before-render orchestration contract (atomic across four SKILL.md mirrors).** Insert an explicit audit step between reconciliation and rendering in `.claude/skills/deep-review/SKILL.md`, `.codex/skills/deep-review/SKILL.md`, `.claude/skills/review-plan/SKILL.md`, and `.codex/skills/review-plan/SKILL.md`. The expected invocation strings (the orchestration-contract test asserts on these):
   ...
   Without `--auto-fix=trivial` the audit still runs as a dry-run preview so `[AUTO-FIXABLE]` only appears from computed `auto_fix_status: "would_apply"`. **Two-layer enforcement (because the audit lines differ per-skill, a single 4-way byte-identical region is structurally impossible — review-plan carries `--plan`, deep-review does not):** (a) `scripts/check-prompt-parity.sh` enforces Claude↔Codex byte-identity for the GENERIC FINDING SCHEMA region, where the cross-runtime schema fragment lives; (b) `tests/parity/test-auto-fix-orchestration-contract.sh` is the authoritative gate for the audit-invocation contract — it asserts, for each of the four SKILL.md mirrors, that (i) the expected invocation string for that skill is present (literal substring match anchored on the script name AND the `--skill` arg AND, for review-plan, the `--plan` arg), and (ii) the audit step appears textually before the render step. Land the four SKILL.md edits + the orchestration-contract test in a single commit so `check-prompt-parity.sh` (schema region) and the orchestration-contract test (audit invocation) are both green at the phase boundary.
...
6. **Kind-specific gate tightening with named adversarial fixtures.** Tighten `docstring_typo`, `import_sort`, and `unused_import` gates in `scripts/apply-auto-fix-code.sh`. Add `tests/auto-fix/test-deep-review-kind-gates.sh` driving the gates against named fixtures (each named in the Test files list above):
   ...
   - `import_sort-accept.jsonl` (pure reorder) and three reject fixtures, all → `rejected_semantic_change`:
     - `import_sort-reject-symbol-change.jsonl` — `before`/`after` binding sets differ (a symbol is added or removed).
     - `import_sort-reject-non-import.jsonl` — the cited line is not an `import …` or `from … import …` statement at all; either side failing to canonicalise as imports is a structural rejection (kept under `rejected_semantic_change` so callers handle all import-kind rejections uniformly).
     - `import_sort-reject-alias-module-swap.jsonl` — the alias is identical on both sides but the module path differs (`import old.module as t` → `import new.module as t`). The canonical binding tuple is `(import-kind, module-path, name|alias)` so a module-path change is a binding-set difference even when the alias is unchanged.
...
7. **Stale v1-renderer compatibility fixture.** Snapshot `scripts/render-reconciled-report.sh` at the pre-Phase-1 v1 commit into `tests/reconciliation/fixtures/render-reconciled-report-v1.sh` (read-only fixture). Add `tests/reconciliation/test-renderer-v1-rejects-v2.sh` asserting the v1 renderer exits non-zero with the message `schema_version mismatch (got 2, expected 1)` (the historical v1 renderer's actual wording) when fed any v2 envelope from `tests/reconciliation/fixtures/auto-fix-v2-*.jsonl`. AC #12 quotes the same wording. The new fixture closes the gap between the AC claim and the implemented test.
...
10. **Post-impl review gates (defines Phase 5 completion).** After items 1-9 land:
    ...
    - (d) Run `/review`, then `/deep-review`, `/update-docs`, and `/security-review` against the final branch state. Findings from each gate are applied to the branch before merge.
    **Phase 5 is complete ONLY when all of these hold simultaneously:** both post-impl `/review-plan` passes return zero Critical / Important findings (or applied fixes re-pass both passes), `/review`, `/deep-review`, `/update-docs`, `/security-review` are green on the final branch state, `test-marker-refresh-post-phase5.sh` is green against the latest accepted marker, and every gate in the Phase 5 Test command + Validation cmd is green.

    **Stop condition (cross-runtime review loop).** Each `/review-plan` round may produce applied findings, which can in turn produce new findings on the next round — without a bound the loop can iterate indefinitely. The stop rule: after one full Claude `/review` cycle followed by one Codex follow-up, the next cross-runtime `/review-plan` gate MUST return zero Critical / Important findings or the residual findings MUST be escalated to the user for a discuss / waive decision rather than being iterated on again. Each gate run records its findings + resolution in `## Issues & Solutions` so the loop's termination is auditable.
...
- **Strict line-anchored apply for review-plan (v2 contract, Phase 5).** `scripts/apply-auto-fix-plan.sh` matches `auto_fix.before` only at the exact `auto_fix.scope` line. The v1 "find unique `before` match anywhere in the file" fallback is removed in Phase 5. Rationale: under v1, a typo in `scope` line could silently land an edit at a different line; making `scope` authoritative forces the lens to emit accurate line anchors and turns drift into a clean `rejected_drift` failure. Paired with triple path equality (`finding.file == auto_fix.scope.path == --plan`), the applier cannot edit a different file or a different line than the one under review. The `Phase 5: Codex review follow-up hardening` item 3 binding test enforces both invariants. **Defence-in-depth uniqueness check:** even when the cited line byte-matches `before`, the applier additionally refuses (also `rejected_drift`) if the same `before` line appears anywhere else in the plan. Rationale: a duplicate `before` line means the lens emission is ambiguous — the same target text is reachable from multiple call sites — and refusing is safer than applying at the cited line. Trivially-common identical lines (blank lines, lone fence delimiters) should not appear as `auto_fix.before` in well-formed lens emissions; if they do, refusing is the correct outcome.
...
- **`import_sort` canonical binding tuple is `(import-kind, module-path, name|alias)`.** The kind gate normalises both sides via `af_canonical_import_records` (in `scripts/lib/auto-fix-common.sh`, shared with the auditor) and compares the resulting sorted-unique multisets. Rationale: an alias-only check (`import old as t` → `import new as t`) would silently pass a module swap; a name-only check would miss module renames. Including the module path makes the tuple structurally accurate to Python's binding semantics. Any side that fails to canonicalise (non-import statement, malformed identifier) is rejected too: `rejected_semantic_change` covers both binding-set differences and "cited line is not an import" cases, so callers handle import-kind rejections under one status. Shared implementation in the lib means the auditor's `would_apply` preview matches the applier's verdict exactly.
...
9. Without `--auto-fix=trivial`, both skills behave identically to today (no edits applied) but the rendered report shows `[AUTO-FIXABLE]` annotations next to every finding with precomputed `auto_fix_status: "would_apply"`. ALL `rejected_*` statuses are surfaced as advisory without the annotation — the current preview-side enumeration covers `rejected_kind`, `rejected_kind_scope`, `rejected_multiline`, `rejected_semantic_change`, `rejected_revar`, `rejected_scope`, `rejected_path`, and `rejected_drift`. The renderer emits `[AUTO-FIXABLE]` only for the exact string `would_apply`, so future additions to the auditor's `rejected_*` vocabulary do not require renderer or AC changes. Verified by `tests/reconciliation/expected/auto-fix-v2-reject-statuses.md`.
...
12. `bash tests/reconciliation/test-renderer.sh` is green against v2 envelopes; `bash tests/reconciliation/test-renderer-v1-rejects-v2.sh` is green (proves the snapshotted v1 renderer at `tests/reconciliation/fixtures/render-reconciled-report-v1.sh` exits non-zero with `schema_version mismatch (got 2, expected 1)` for every v2 fixture); v1-style JSONL findings still parse (upgrade in-flight); v2 JSONL findings with malformed `auto_fix` are rejected with clear error.
...
<!-- reviewed: 2026-05-17 @ 110757d9e7f04345b9f9f8a630d841bb77d8f83a -->
...
### Claude cross-runtime gate (item 10(b), 2026-05-17)

Phase 5 implementation landed in commit `a033324` (Codex-conducted). The Codex `/review-plan` post-impl pass mandated by item 10(a) was folded into this Claude cross-runtime gate because `git show a033324 -- docs/dev_plans/20260515-feature-review-auto-fix-tier.md` returned empty — the Phase 5 implementation did not modify any line above the marker, so the marker (`823d6da5…`, written by `13c9109` before impl) remained structurally valid against the post-impl branch state. A separate Codex post-impl `/review-plan` run would have reviewed identical contract bytes, so the cross-runtime gate alone satisfies the intent of both 10(a) and 10(b).

The gate returned 3 Important and 5 Minor findings; all applied:

- **Important (architecture)** — Plan item 4 originally said "wrap audit invocations inside the byte-identical region enforced by `check-prompt-parity.sh`, not a separate substring grep." This was structurally infeasible because deep-review and review-plan audit lines differ (`--plan` arg). Item 4 is now reframed as two-layer enforcement: `check-prompt-parity.sh` covers the GENERIC FINDING SCHEMA byte-identical region (cross-runtime); `tests/parity/test-auto-fix-orchestration-contract.sh` is the authoritative gate for the per-skill audit invocation contract.
- **Important (sequencing)** — Item 10(a) post-impl Codex `/review-plan` pass was not separately recorded. Now explicitly folded into 10(b) (this entry), with the structural justification above. Marker remains valid against post-impl branch state because Codex's impl commit did not touch the contract section.
- **Important (spec/testing)** — AC #12 and Phase 5 item 7 previously quoted `schema mismatch: got 2, expected 1`; the actual v1-renderer fixture emits `schema_version mismatch (got 2, expected 1)`. Plan text updated to quote the historical wording the fixture preserves.
- **Minor** — Architecture Decision "Strict line-anchored apply for review-plan" expanded to document the defence-in-depth uniqueness check (a `before` line appearing elsewhere in the plan → `rejected_drift` even when the cited line byte-matches), making the binding test case 3 contract explicit. Comment added in `scripts/apply-auto-fix-plan.sh` for the same. Plan item 4 wording corrected from "regex match" to "literal substring match" (matches the awk `index()` implementation). Atomic-commit groupings (items 2, 3, 4) collapsed into Codex's single Phase 5 commit — trivially atomic; documented as acceptable. Progress checkbox tick `[x]` precedes this final gate; left as-is since findings here are addressable plan-text fixes. Two-arg byte-identity test uses authored expected rather than a captured pre-change snapshot — accepted as low risk.

Phase 5 completion criteria (item 10) status: post-impl reviews returned only Important/Minor findings, all applied in this branch; `bash tests/auto-fix/test-marker-refresh-post-phase5.sh` and the full Phase 5 Test command exit zero against the latest accepted marker.

### Codex post-Claude /review hardening (2026-05-17, commit a033324 + 004c3e3)

Claude `/review` on Phase 5 (commit `09651bb` flagged: blank-line emission in `plan-scope-detect.sh --stack`, duplicate `run_plan_applier` in test helpers, clean-index guard asymmetry intentional documentation, `|| true` silent-swallow on detector exit-2). Codex followed up in `004c3e3` with substantially more hardening: auditor enlargement (+378 lines) covering review-plan path binding, symlink rejection, detector-failure handling, and deep-review kind-specific gate previews; `import_sort` semantic tightening in the code applier (+70 lines) including alias-module-swap and non-import rejection; indented-fence handling in `plan-scope-detect.sh`. Marker refreshed to `f5f84525…`.

### Claude cross-runtime gate on Codex 004c3e3 (2026-05-17)

`/review-plan` cross-runtime gate returned 8 Important + 4 Minor findings against `004c3e3`, plus codebase-claims clean. All applied to this branch:

- **Important (architecture)** — auditor `assert_no_symlink` / `canonical_existing_path` re-implementations consolidated into `scripts/lib/auto-fix-common.sh` as `af_assert_no_symlink` / `af_canonical_existing_path`. The lib's symlink walk now accepts an optional `<root>` arg to bound the walk so the auditor can scope to `AUDIT_ROOT` (fixtures under symlinked prefixes like macOS `/tmp` → `/private/tmp` were rejecting otherwise-valid paths). Auditor now sources the lib and calls `af_canonical_existing_path "$path" "$AUDIT_ROOT"`.
- **Important (architecture)** — `FORBIDDEN_HEADINGS` array + `heading_is_forbidden` + `stack_is_forbidden` moved to the lib as `AF_FORBIDDEN_HEADINGS` + `af_heading_is_forbidden` + `af_stack_is_forbidden`. Single source of truth; auditor and applier wrap the lib calls. Closes the drift bug class where auditor `would_apply` could disagree with applier `rejected_scope`.
- **Important (architecture)** — `canonical_import_records` + `same_import_symbol_set` (~65 lines verbatim duplication) moved to the lib as `af_canonical_import_records` + `af_same_import_symbol_set`. Both code applier and auditor `import_sort` gate now share one implementation.
- **Important (sequencing)** — Phase 5 item 10 now includes (d) `/review`, `/deep-review`, `/update-docs`, `/security-review` as part of the Phase 5 completion criteria, plus an explicit Stop condition for the cross-runtime review loop: after one Claude `/review` + one Codex follow-up, the next gate must produce zero Critical/Important or escalate to user discuss/waive.
- **Important (spec/testing)** — AC #9 generalised: rather than enumerating two specific reject statuses, the AC now says ALL `rejected_*` statuses are surfaced as advisory without `[AUTO-FIXABLE]`; the renderer's emit-only-for-would_apply rule is the invariant. Future additions to the auditor's reject vocabulary do not require AC or renderer changes. New fixture `tests/reconciliation/expected/auto-fix-v2-reject-statuses.md` proves the contract (six findings: one `would_apply` → `[AUTO-FIXABLE]`, five `rejected_*` → no annotation).
- **Important (spec/testing)** — Phase 5 item 6 now spells out the three `import_sort` reject cases (symbol-set diff / non-import / alias-module-swap), with a new Architecture Decision documenting the canonical binding tuple `(import-kind, module-path, name|alias)` and why all three live under `rejected_semantic_change`.
- **Important (spec/testing)** — symlink-plan binding test strengthened: after asserting the out-of-repo target survives the rejection, the test removes the target and re-runs; asserts the applier still exits non-zero AND the output does NOT contain `No such file or directory` (kernel ENOENT), proving the applier rejects pre-read rather than reading and deciding not to write.
- **Important (spec/testing)** — `test-deep-review-kind-gates.sh` now asserts the auditor's `would_apply` preview for `unused_import-accept` (previously only the rejects + docstring/import accepts were asserted). The audit-before-render contract is now provably exhaustive across the allowlist accept paths.
- **Minor** — `scope_parts` / `parse_plan_scope` consolidated into `af_parse_plan_scope`; the applier and auditor wrap it. Indented-fence-parent regression in `test-review-plan-scope-forbid.sh` now exercises the auditor directly via the new `auditor status=rejected_scope` assertion in `e2e_rejected_scope` (was previously implicit through the applier's re-verification).

**Pre-existing renderer empty-field bug fixed in same branch.** The awk + bash renderer's TSV decode previously collapsed adjacent empty fields when `IFS=$'\t'` was whitespace-separated, causing the next non-empty value to bleed into empty slots and (worst case) silently suppressing the `[AUTO-FIXABLE]` annotation on `would_apply` findings whose preceding fields were empty. Fix: switched the intermediate inter-field separator from `\t` to `\x1f` (ASCII Unit Separator, non-whitespace) in both the jq and awk renderer paths plus the bash `read -r` consumer. Tag prefixes (`FINDING\t`, `RELATED\t`) stay tab-separated so the downstream tag-filter awk still splits tag from data. Content escapes (`\t`/`\n` inside fields) unchanged so multi-line evidence still round-trips. New `tests/reconciliation/expected/auto-fix-v2-empty-fields.md` fixture exercises three empty-field shapes (both empty / suggestion-only empty / evidence-only empty) and asserts `[AUTO-FIXABLE]` appears for all three would_apply findings.

Marker refreshed at end of this fix batch. Test gate (Phase 5 Test command + renderer tests) exit 0.
