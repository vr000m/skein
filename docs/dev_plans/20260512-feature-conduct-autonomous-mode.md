...
**Created**: 2026-05-12
**Last revised**: 2026-05-13 (post-Codex re-review: drop --split-mirror-commits; add ConductResult ci-parity fields, incompatible_with_pre_vN enforcement, Phase 4 handoff gate, factual fixes)
...
- **End-of-plan CI-parity gate is main-runtime-orchestrated with a hardened state-machine protocol.** Conduct does not directly call the runtime worker tool (`Agent` on Claude, `spawn_agent` / `wait_agent` / `close_agent` on Codex). When the gate dispatches, conduct:
  ...
  3. SKILL.md instructs the main runtime orchestrator to: ensure `.conduct/` exists (use exported `_ensure_safe_conduct_dir` helper), dispatch the ci-parity worker with the harness-native mechanism and `ci-parity-prompt.md`, capture the worker's final JSON, **atomically** write it to `<repo-root>/.conduct/ci-parity-result-<plan-id>.json` via the `_safe_write_text` helper in conductor.py (grep for the symbol) (tmp file + `os.rename`), then re-invoke `/conduct --resume <plan>`.
...
- **`plan-id` derivation is shared with state-file naming.** `plan-id = sha1(abs(plan_path).encode()).hexdigest()[:12]`. Stored in `state["plan_id"]` on first state write so the main runtime orchestrator reads it from state, not by re-computing. State-file naming (`.conduct/state-<basename>-<plan-id>.json`) reuses the same derivation; the plan reuses the existing conduct convention rather than inventing a new one. If conduct's current state-file naming uses a different scheme, the implementer reconciles (Phase 3 first task) — either by adopting the existing scheme as the canonical `plan-id` or by introducing both fields with documented semantics.
...
- `ConductOptions` (conductor.py:86): keep `max_iterations: int = 3` (legacy). Add `max_iterations_ceiling: int = 8`, `stall_threshold: int = 2`, `no_progress_detection: bool = False`. Startup validator: if `stall_threshold >= max_iterations` AND `no_progress_detection is False`, emit warning.
...
- **Extend `ConductResult` dataclass.** Add typed-optional `next_action: Optional[str] = None` and `request: Optional[dict] = None` fields. These are the carrier for the `awaiting_ci_parity` handoff from conduct to the main runtime orchestrator. Both runtimes mirror the extension (Claude-side already landed in commit eb65eba; codex-side mirrors it under this phase's codex-driven run).
...
**Test command:** `just check-sync && just check-prompt-parity && uvx pytest .claude/skills/conduct/tests/ -v && (cd .codex/skills/conduct && uv run --with pytest python -m pytest tests/ -q) && bash tests/parity/test-prompt-parity-extended.sh`

**Handoff gate (must pass before any check/test below runs):** Block until both the claude-side and codex-side mirror commits have landed on the branch. Verify via `git log` that for every phase `N` (1, 2, 3) there exists a `conduct(phase N): ...` commit from each runtime (claude-driven and codex-driven), or an equivalent boundary commit from each runtime touching the corresponding `.claude/skills/conduct/` and `.codex/skills/conduct/` paths. The parity and sync checks below assume both runtimes have completed their phase work; running them earlier will spuriously fail on the lagging-mirror state.

- Run the full Claude conduct test suite and the Codex conduct suite from a clean checkout.
...
- `.claude/skills/conduct/SKILL.md` — Step 6 rewrite (stall-aware dual cap, helper-owns-increment invariant); Step 9 split; new "CI Parity Gate" section (detection priority, override, escape hatch, single-exit, four-sub-case result-file handling, lock-release contention, `.conduct` creation, atomic-write protocol); State File section (v1/v2 schemas, unknown-≥1 forward-compat contract, `incompatible_with_pre_vN` hard-fail rule, plan_id derivation, Phase 1 known subset `{stall_signatures, stall_count, autonomous, plan_id}`); preflight section note about custom `--lint-check`; pre-commit-hook exemption note for impl commit during Phase 3 (verified at 298 lines today).
- `.claude/skills/conduct/conductor.py` — `ConductOptions` extension; add typed-optional `next_action: Optional[str] = None` and `request: Optional[dict] = None` fields to `ConductResult`; `_check_iteration_bound` helper (owns increment); centralised wiring at three call sites; explicit phase loop in `conduct()`; loader hard-fail on `incompatible_with_pre_vN`; `_dispatch_ci_parity_if_eligible` helper called from both exit points; `_consume_ci_parity_result` helper (shared by test and production paths); preflight branch for `awaiting_ci_parity` status (four sub-cases); `_ensure_safe_conduct_dir` exported for orchestrator use; `--skip-ci-parity` short-circuit; phase-start reset guard at line 541; `_handback` unchanged; `_persist_state` schema_version field; new dataclass `CIParitySpawnRequest`; `plan_id` derivation reused from existing state-file naming convention.
...
- **`_ensure_safe_conduct_dir` exported helper.** The main runtime orchestrator uses this before writing the result file; conduct also uses it internally.
...
- **CI-parity result handoff file.** `<repo-root>/.conduct/ci-parity-result-<plan-id>.json`. Atomic write via the `_safe_write_text` helper in conductor.py (grep for the symbol) — tmp file + `os.rename`. Consumed file moved to `.consumed-<unix>.json` (not deleted).
...
16. `just check-sync` and the extended `just check-prompt-parity` (now covering `*-prompt.md` for every managed skill) green after both runtime mirrors have landed (Phase 4 handoff gate). The impl commit before each mirror commit deliberately leaves mirrors lagging; preflight is verified NOT to call `check-sync` and `phase-3-impl-commit-lands-with-hooks-enabled-in-intermediate-state` confirms the Phase-3 impl commit lands with hooks on even in the lagging-mirror state. Codex mirror acceptance also requires `(cd .codex/skills/conduct && uv run --with pytest python -m pytest tests/ -q)` to pass after the Codex-native remap.
...
20. SKILL.md documents all new flags, state fields, helpers, protocols, contracts, and bootstrap notes.
21. `/deep-review`, `/review`, `/security-review` clean on the final PR.
