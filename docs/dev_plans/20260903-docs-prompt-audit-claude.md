...
- **AD-4 The Codex delegation clause is an operative Codex-only gate, kept as an intended divergence; the untrusted-input wrapping is not, and both mirrors carry it.** The C9 rewrite of the ten sites (`spec-compliance:45` in C8, nine in C9) turned the old "if delegation is available and explicitly allowed" hedge into a rule: delegate only at top level or under an orchestrator's explicit authorisation, never spawn a nested worker, fall back to the main context when `spawn_agent` is unavailable. The Claude mirror carries no twin on purpose: Claude subagents have no Agent tool, conduct's implementer prompt forbids nested spawns and deep-review states one level of delegation, so the invariant is enforced by the harness rather than restated per skill. The five sites (content-draft:42, content-review:46, rfc-finder:29, spec-compliance:37, update-docs:27) are harness-divergent by decision, not drift (decided after gauntlet round 6; see Findings). Effort-annotation counts per file are preserved. The `<untrusted-content>` wrapping of every user-supplied placeholder, with the closing-tag rewrite rule, is a prompt-injection data boundary that no harness mechanism provides on either side, so it is not covered by the delegation rationale: the Claude twins carry the same block byte-for-byte (ported after gauntlet run 3, when the security and architecture lenses had re-reported the gap in every round of run 2). Only the delegation clause remains Codex-only.
...
<!-- reviewed: 2026-09-04 @ c5855cd0df606f44281a5cf833ba98bb66507d45 -->
...
- **Review-gauntlet rounds (post-Phase F):**
  ...
  - Round 12 (confirm) — 74be775, 18016fd, 61c65ff: `perl` restored to both plans' dependency lines (`test-spawn-tiers.sh` still uses `perl -0777`); the five Codex untrusted-input clauses say a literal closing tag inside a value is content; the Codex plan's Learnings bullet matches AD-8's lag-window rule. Quarantined: C-F16 at two Claude sites, `.claude/CLAUDE.md:96` re-report (boolean contract and Explore routing remain in item 2 of the same section), the pending run-2 decision line.
  - Round 13 (confirm) — 47e9bc5, 5dcbc91, 49405f2: Outcomes line defers round commits to this ledger; update-docs base-branch allowlist admits `@`, `+`, `,` (both mirrors); the five Codex untrusted-input clauses make the boundary delimiter-safe by rewriting a literal `</untrusted-content` inside a value before substitution. Quarantined: the release worked example (B-18, kept as illustrative), the pending run-2 decision line.
  - Round 14 (confirm) — 3a2a988, 9cb4d78: `.claude/CLAUDE.md:30` no-plugin fallback drops `/deep-review` (a skein skill) and both plans' D-F05 record follows; update-docs base-branch allowlist refuses a bare `@` (git reads it as HEAD; both mirrors); Codex plan Learnings scope the fresh-thread-review claim to rounds ≤9. Quarantined: C-F16 re-reports ×3, the `**Review Gates:**`/`**Review Gates**:` token split (pre-existing, test-pinned; Follow-up Work), the lens-prompt delimiter escaping (Follow-up Work), the pending run-2 decision line. Ledger: count 3 for the second consecutive round at epoch minimum 3 → **non-converge** (K=2 stall; loop counter 8 of 10).
  - Run 3 (operator-invoked after the PR opened; scope `plugins/skein/` only, Codex adversarial gate skipped by instruction, so every round carries `--unresolved 1` and `success` was unreachable by construction) — Round 15 (full): c3a73c4 fan-out Step 2 reads `agent-prompt.md` through the path anchor; the worker's write-containment sentence regains its `IMPORTANT:` framing (both mirrors). Round 16 (confirm): 558b164 spec-compliance Step 3 says to work only from the fetched text (both mirrors); plan-view `footer_script_path()` shortens the home directory in the regenerate footer, two tests pin both branches. Round 17 (confirm): 6e6939f fan-out spells `${CLAUDE_PLUGIN_ROOT}/skills/fan-out/...` in every call because shell variables do not persist between Bash tool calls (Codex keeps its env-exported `$SKILL_DIR`); plan-view escapes `SCRIPT_PATH`/`PLANS_DIR`/`PLANS_DIR_SHORT` through `_esc()`; deep-review's inline worktree-detection sentence states a reason that holds. Counts 2, 2, 4 → **non-converge** (K=2 stall at epoch minimum 2). Nothing quarantined.
...
Phases L1–L10 and F landed as one boundary commit each (cf268f2 … 11faf8c), aligning `plugins/skein/` and `.claude/CLAUDE.md` to the Codex mirror text. The auto-chained review-gauntlet ran six rounds and stopped on the K=2 stall rule with decision non-converge. Every actionable finding was fixed; the stall is driven by re-reports of items the user has to decide. Those decisions (C-F16 as an intended Codex-only divergence, Phase R removing the fan-out status apparatus, rubric vocabulary kept) were applied and a second gauntlet run started at round 7; its rounds are listed under Findings. Run 2 decision: **non-converge** after eight rounds (7–14, all confirm passes; actionable counts 25, 12, 11, 4, 4, 3, 3, 3). Every actionable finding in every round was fixed; the last three rounds each surfaced three new small documentation or consistency items rather than re-reports, so the K=2 stall rule fired at count 3 instead of the count reaching zero. Nothing actionable is open at HEAD; the quarantined residue is the sanctioned C-F16 divergence (re-reported by the security and architecture lenses every round as an input-isolation gap distinct from delegation depth), two items logged under Follow-up Work, and this Status line. Run 3 (rounds 15–17, Claude mirror only, Codex gate skipped at the operator's instruction) fixed all eight findings it raised and also ended **non-converge** on the same stall rule; with the Codex gate skipped, `success` was unreachable regardless.

### Outcomes

- Gauntlet rounds (reconciled / fixed / quarantined): R1 13/10/3, R2 14/11/3, R3 9/9/0, R4 8/7/1, R5 9/6/3, R6 13/6/7; run 2: R7 25/19/6, R8 12/10/2, R9 11/6/5, R10 7/4/3, R11 11/4/7, R12 7/3/4, R13 5/3/2, R14 10/3/7; run 3: R15 2/2/0, R16 2/2/0, R17 4/4/0. Round commits are named per round in the Findings ledger above.
- Fixes landed by the gauntlet beyond the plan's hunks: rfc-finder adoption-claims rule; fan-out path anchor repaired (the Claude side spells `${CLAUDE_PLUGIN_ROOT}/skills/fan-out/...` in every command plus a one-time file-existence check, since the harness substitutes the bare token, does not export a variable, and shell state does not survive between Bash tool calls; `${SKILL_DIR:?}` on Codex, where it is env-exported); stale `.claude/skills` and `.codex/skills` paths replaced across conduct, dev-plan, plan-view (SKILL.md, generate.py comment, test fixture, ci-parity docstring) and `scripts/check-prompt-parity.sh`; plan-view rich-mode invocations use the harness path idiom; Codex update-docs recipe no longer reads an unassigned variable; project CLAUDE.md decision rule wording; conduct schema_version rule made conditional; flag-only C-F15 resolved.
...
- Post-Codex alignment (2026-09-04, f6c53b2..c2141c3): after the Codex gauntlet hit its 10-round cap, the Claude mirror re-adopted the reverted wrapping port (b75331c had undone 0a064f7), the fan-out and conduct worker-prompt hardening, the `PHASE_LABEL_DISPLAY`/`PHASE_LABEL_JSON` split and the plan-view render-sha changes; the Codex mirror and `tests/` were not touched. Green: `check-sync`, `test-spawn-tiers.sh` (217), `test-prompt-parity-extended.sh` (29), `test-no-manual-apply-fallback.sh` (13), plan-view suites (69 Claude / 68 Codex). Two cross-ownership blockers remain: (1) `check-prompt-parity` still drifts on `fan-out/test-writer-prompt.md` because the Codex twin's "Dormant Dispatch Contract" section (pinned by `test-spawn-tiers.sh:512`) has no truthful Claude twin, the Claude template being live; needs a normaliser carve-out or Codex rewording. (2) `tests/gauntlet/test-goal-injection.sh` pins `restate.{{PHASE_GOAL}}` / `means.{{PHASE_GOAL}}` in the Claude conduct prompts, but the Codex rewrite ends that sentence with `authoritative.` (the no-leading-space glue invariant still holds); the pattern needs updating on the Codex side. The Codex `plan-view/generate.py` is not ruff-formatted; the Claude copy is, so the generators differ by formatting only. Codex closed all three in 2d03052, 9b39b2d and fcf778e; at fcf778e every gate is green, including `gauntlet-tests` with no A11 flake.
- Decided after round 6: C-F16 recorded as an intended Codex-only divergence (AD-4 rewritten, no Claude twin); revised after run 3 on the lenses' standing input-isolation argument: the delegation clause stays Codex-only, the untrusted-input wrapping is ported to the Claude twins (AD-4 split); the R6 prompt apparatus removed on this branch (Phase R below); the Managed-Agents rubric vocabulary kept as descriptive wording. Dismissed with evidence: the grill guard claim (round 4), the conduct rename-clause claim (round 5), the lag-window rule claim (round 6, the plan text already states the recipe is red inside the window).
