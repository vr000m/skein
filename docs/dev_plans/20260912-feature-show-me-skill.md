...
**Quarantined finding (review-gauntlet round 1, 2026-09-13):** Codex's adversarial gate flagged that neither plugin manifest's `version` was bumped nor was a `CHANGELOG.md` entry added, citing `AGENTS.md:134`'s single-PR release path. That page documents two valid paths — fold the bump into this PR, or accumulate under `[Unreleased]` for a separate `skein:release` cut — and this repo's actual practice for the last three versions (0.7.0/0.7.1/0.7.2, each a distinct `release: X.Y.Z` commit) used the second path, with `[Unreleased]` sitting empty between releases. Bumping the version here would conflict with that established practice and preempt a release-timing decision that belongs to the maintainer, not to a fixer applying a gate finding — so this is quarantined rather than applied. Resolve by running `skein:release` when ready to cut a release including this change, not by editing the manifests in this PR.
