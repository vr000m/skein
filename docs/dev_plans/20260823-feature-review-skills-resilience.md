...
- **Three r11 findings closed WITHOUT a code change (2026-08-25, deep-review r11 / F9, F11, F6)**: recorded here so each verdict is auditable rather than silently absent from the diff.
  - **F9 — DEFERRED, not rejected.** `plugins/skein/skills/review-gauntlet/lib/convergence-ledger.sh:769-812` is the cross-round `fixed_keys` promotion machine written as one inline single-quoted jq program with escaped comments; extracting it to `jq -f` would improve readability. Deferred because it is a readability-only rewrite of the most safety-critical logic in the ledger, with no behaviour change and no new coverage, proposed for the same round as F5's validation change to its caller — the blast-radius pattern this repo's CLAUDE.md warns against. It does not become "separately testable" without a CLI seam the finding does not design. Carry to a round that changes nothing else in that file.
  ...
  - **F6 — ANALYSIS ERROR, no defect.** The finding reports that a NUL byte in a `--json-stdin` payload is unguarded. `scripts/persist-lens-result.sh:494-501` already applies the NUL rule to all nine scalar fields, with the reason stated at `:488-490` (NUL is the extractor's own delimiter); `--json-file` assigns `STDIN_JSON` at `:460`, upstream of the same gate, so both transports are covered. The finding describes the pre-round-5 state — `scripts/lib/lens-common.sh` (round 4 dropped the rule, round 5 restored it) records that history. Nothing to fix.
