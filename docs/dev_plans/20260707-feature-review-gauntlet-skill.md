...
- **Session-resume is not implemented.** The convergence ledger persists `{loop_counter, rounds}` per round, but that state feeds only the in-run stall-detection math — nothing reads it back to resume an interrupted session after an API drop. Flagged as the top-priority gap in the 2026-07-06 Claude Code Insights report; tracked in [`20260710-feature-review-gauntlet-resume.md`](20260710-feature-review-gauntlet-resume.md).
