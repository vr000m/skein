...
**Rich-page navigability (later follow-on, branch `feature/plan-view-rich-navigable`).** The `--rich` pages (`plan-<slug>.rich.html`) shipped orphaned — nothing linked *to* them and they linked *to* nothing, so they were only reachable by typing the path. This branch wires the two tiers together: each dashboard card gets a `rich →` link and each per-plan page header a `rich view →` link to its rich counterpart, and the rich pages (both the `assemble_rich_sections` output and the single/section subagent prompts) gain a `← Plan View · deterministic view` breadcrumb back to `index.html` and `plan-<slug>.html`. The forward link is emitted **unconditionally** from the deterministic `plan-<slug>.rich.html` filename mapping, so a rich view becomes navigable the moment it is generated and **no `compute_render_sha` change is needed** (the link text is identical every run; idempotent reruns still no-op). Consequence documented in SKILL.md: because `render_sha` covers plan/corpus/git inputs but not the HTML *template*, this template change requires a one-time `--force` (or fresh `--out`) against pre-existing outputs. Two review fixes from a Codex pass: the card link was moved out of the `-webkit-line-clamp:2 / overflow:hidden` title box into the unclamped `.meta-row` (it was clipping on long titles), and `{{RICH_HREF}}` now routes through `_esc(plan.slug)` to match the card-link escaping (regression test `test_plan_page_rich_href_escapes_slug`). New tests in `test_links.py`; `assemble_rich_sections` back-links asserted in `test_sections.py`. `.codex/` mirror kept byte-identical.
