...
- Wire the four fields into Step 3 (Compose Title and Body): `title_format` picks the title shape; `compare_line_label` substitutes the fixed label text into the existing compare-line template (`**<label>:** <url>/compare/...`), `"none"` omits the line entirely regardless of PREV; `excluded_sections` drops matching verbatim `###` subsections from the copied CHANGELOG body before composing (a heading absent from the fetched CHANGELOG section is a no-op, reported as such in the Step 4 line-item; an exclusion set that empties the body entirely is a Step-1.4-style hard stop, not a silent empty release); `whats_new: false` forces suppression, overriding the existing recover/preserve-absence default; `whats_new: true` is explicitly not an override — it is a no-op equal to leaving the field unset, since it only recovers/preserves an existing summary and never forces drafting new prose.
...
- **Follow-up plan spun off (2026-09-17):** a further `/code-review xhigh --fix` pass on the templated `release/SKILL.md` (post-merge, gauntlet rounds 1–3 already applied) found 15 findings, 14 unfixed — nearly all traceable to the same root cause: deterministic template-read/jq-gate/marker-resolution logic re-specified as prose at ~5 call sites × 2 mirrors instead of living in a single script, plus no progressive-disclosure split so the template subsystem's ~60K tokens load even on untemplated (the common-case) `/release` cuts. Structural fix tracked in [`docs/dev_plans/20260917-refactor-release-skill-structure.md`](20260917-refactor-release-skill-structure.md) — scripts/ extraction, references/ split, A2/A2.5 decoupling, and parity-test anchor stability. That plan explicitly does not re-resolve the 14 unfixed content findings themselves; it only relocates the logic they point at (see its "Related Work" section for the explicit link).
