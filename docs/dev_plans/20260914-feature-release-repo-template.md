...
- Marker present and valid → classify against that pinned blob content, stripping the marker itself and applying that blob's `excluded_sections` before the exact-bytes CHANGELOG comparison (same recovery logic Phase 1 added to A2/Step 3.1).
...
| Step 3.1 / Audit A2 body-recovery parsing | Phase 1 (marker/label-aware recovery, landed with Step 3 wiring) | Phase 1 re-sync, Phase 2 A2 classification | Both must strip the trailing marker and apply the active `excluded_sections` before comparing bytes — a mismatch here misparses a valid templated release as malformed/drifted |
| A2 per-candidate `gh release view` fetch | Phase 2 (Audit A2) | Phase 3 (no-template dry-run) | Phase 3 reuses A2's already-fetched `name`/`body` for the highest 2-3 `T=R=C=✓` candidates; issues no new `gh` calls |
