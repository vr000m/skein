...
4. **Phase 0 findings — chaining map, chaining-survival test, context-load test. Recorded 2026-07-12.**
   ...
   **This is Phase 0's single most important open question, not yet a closed finding.** If (c) is correct, every hard Axis-1 edge in the table above would genuinely break if its callee were marked user-invoked. But the confidence here rests on doc-inference, not observation — Phase 2 (below) adds a concrete gate to close this gap before disabling further skills. Provisionally, this is why `review-gauntlet`, `deep-review`, `dev-plan` (and probably `conduct`) must stay model-invoked — pending that empirical check.
...
<!-- reviewed: 2026-07-12 @ 39f9b744d279c97cadb777772ce4f295544b7028 -->
...
**Open item flagged, not resolved here:** Context item 4 (line 79) states "Phase 2 (below) adds a concrete gate to close this gap before disabling further skills" — but Phase 2, as actually executed, applied `disable-model-invocation: true` to `plan-view` and did not add any empirical verification gate. This doesn't block the current scope (`plan-view` has zero confirmed hard Axis-1 inbound edges, so the unresolved (c) question about literal-text chaining survival doesn't bear on it), but the Context prose's forward-reference to a gate that doesn't exist is a loose thread worth resolving in a follow-up if more skills are ever proposed for disabling.
