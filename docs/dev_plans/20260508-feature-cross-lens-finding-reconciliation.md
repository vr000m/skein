...
Refinement to the **"signature is `(file, line, category)` only"** decision above. A finding with BOTH an empty `file` AND an absent `line` (normalised to the `-1` sentinel) has no location anchor, so the `(file, line, category)` signature carries no structural identity — every such finding would otherwise share one `("", -1, category)` signature and collapse into a single record, silently dropping distinct findings. This was latent for any file-less finding and became routine when `/review-plan` added the `assumptions` lens (its findings — claims about external/backend behaviour, semantics, data shape — typically have no in-repo location).
