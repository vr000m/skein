...
- `scripts/reconcile-findings.sh` — Phase 1. Single source of truth for reconciliation logic. Stdin: JSON-Lines findings. Stdout: canonical reconciled JSON report. Uses `jq` if present, falls back to `awk`/`sort`/`uniq` otherwise (no new dependencies). Emits `{schema_version: 1, summary: {raw, merged, unique, related, dropped}, findings: [...], related: [...]}`.
...
|------|---------------|---------------|----------|
| `scripts/reconcile-findings.sh` (single source of truth) | Phase 1 | Phase 2 harness, Phase 3 orchestrator step | Stdin: JSON-Lines `{lens, severity, category, file, line, summary, evidence, suggestion}`. Stdout: canonical JSON `{schema_version: 1, summary: {raw, merged, unique, related, dropped}, findings: [...], related: [...]}`. Sort: severity → category → file → line → sorted lenses. |
...
- `scripts/reconcile-findings.sh` — single source of truth for the merge rule, canonical sort, related-callouts, and the `schema_version` and `summary.{raw, merged, unique, related, dropped}` envelope. Uses `jq` when available; falls back to a hand-written awk parser that handles `\"`, `\\`, `\n`, `\t` escape sequences.
...
- **Deep-review Architecture-Important: `summary.unique` redundant** — *resolved 2026-05-10*: redefined `summary.unique` as the count of signatures reported by exactly one lens (single-source); previously it was a redundant copy of `len(findings)`. New invariant `merged + unique == len(findings)` is asserted at runtime in `scripts/reconcile-findings.sh` (exit 2 on mismatch). The reconciliation envelope has not shipped outside this branch (envelope `schema_version` stays at **1**, defined to carry the corrected semantics); all 11 envelope + 11 rendered goldens regenerated; 4 inline expecteds in `test-reconciler-unit.sh` updated; GENERIC FINDING SCHEMA AND MERGE block in all 4 SKILL.md mirrors documents the corrected semantics.
- **Deep-review Architecture-Minor: `emit_related` cross-file gap** — *resolved 2026-05-10*: cross-file related pairs are NOT representable in the schema by construction (`related_tsv` keys on `(file, line)` only). Made the invariant explicit: added an awk assertion in `emit_related_pretty` that fails with exit 2 if a related pair ever crosses `(file, line)`, plus prose in the GENERIC block stating "cross-file related pairs are NOT representable in this schema (the renderer asserts this invariant)."
- **Envelope `schema_version` bump 1 → 2** — *superseded 2026-05-17 in PR #23 (`feature/review-auto-fix-tier`), commit `b259021`*: the auto-fix tier required the reconciliation envelope to carry per-finding `auto_fixable` metadata and the renderer's intermediate TSV separator was switched from `\t` to `\x1f` to preserve empty fields. `ENVELOPE_SCHEMA_VERSION` was bumped from 1 to 2 in both `scripts/reconcile-findings.sh` and `scripts/render-reconciled-report.sh`. Any prose above that still reads "schema_version: 1" reflects the original Phase 1 design; the canonical envelope version on `main` after PR #23 is **2**.

## Addendum — 2026-06-03: unanchored findings are non-mergeable

Refinement to the **"signature is `(file, line, category)` only"** decision above. A finding with BOTH an empty `file` AND an absent `line` (normalised to the `-1` sentinel) has no location anchor, so the `(file, line, category)` signature carries no structural identity — every such finding would otherwise share one `("", -1, category)` signature and collapse into a single record, silently dropping distinct findings. This was latent for any file-less finding and became more visible when `/review-plan` added the `assumptions` lens: external/backend behaviour, semantics, data shape, unread contracts, and environmental claims are sometimes not anchorable to a concrete source line.

Fix: `reconcile-findings.sh` assigns each unanchored finding a per-row uniqueness discriminator (parse column 14) folded into the merge signature. Unanchored findings therefore never merge into one another and are excluded from the `related` cross-reference pass. Anchored-finding merge/related behaviour (the cases checked above) is byte-for-byte unchanged — the discriminator is empty for every anchored finding. Regression coverage: `tests/reconciliation/test-reconciler-unit.sh` cases `unanchored-findings-do-not-collapse` and `unanchored-different-category-no-related`. The change ships identically to all five bundled copies via `scripts/bundle-appliers.sh`.
