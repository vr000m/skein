...
- **Deep-review Architecture-Important: `summary.unique` redundant** — *resolved 2026-05-10*: bumped envelope `schema_version` to **2**; redefined `summary.unique` as the count of signatures reported by exactly one lens (single-source). New invariant `merged + unique == len(findings)` is asserted at runtime in `scripts/reconcile-findings.sh` (exit 2 on mismatch). Renderer's `EXPECTED_SCHEMA_VERSION` bumped to 2; all 11 envelope + 11 rendered goldens regenerated; 4 inline expecteds in `test-reconciler-unit.sh` updated; GENERIC FINDING SCHEMA AND MERGE block in all 4 SKILL.md mirrors documents the new semantics + the v1→v2 rationale.
