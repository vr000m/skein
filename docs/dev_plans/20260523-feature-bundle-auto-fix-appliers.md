...
- **A3 — bare reconcile example marked documentation-only (commit `02822b5`).** Added an inline, mirror-neutral `# documentation only …` note above the bare `scripts/reconcile-findings.sh` example inside the GENERIC FINDING SCHEMA block, identically across all four SKILL.md copies, so the block stays byte-identical across the four mirrors (cross-mirror equality is enforced by the phase-4 GENERIC-block tamper checks in `tests/parity/test-prompt-parity-extended.sh`; a bare hash is intentionally not pinned here because the value depends on the extraction method — marker inclusion, trailing newline, digest algorithm). Forestalls a future "anchor all invocations" sweep from anchoring the example and breaking GENERIC-block parity.
